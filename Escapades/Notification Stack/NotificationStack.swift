 ...
 */
public struct NotificationStack<Data, Content>: View where Data: RandomAccessCollection, Data.Element: Identifiable, Content: View {
    ...
    private func xOffset(for index: Int) -> CGFloat {
        ...
        let padding = 35.0
        let x = (CGFloat(index) - currentIndex)
    ...
    private func yOffset(for index: Int) -> CGFloat {
        let topCardProgress = currentPosition(for: index)
        let padding = 35.0
        let y = ((CGFloat(index) - currentIndex) * padding)
//        if topCardProgress > 0 && topCardProgress < 0.99 && index < (data.count - 1) {
//            return y * swingOutMultiplier(topCardProgress)
//        }
        return y
    }
...
struct CardStackView_Previews: PreviewProvider {
    ...
    struct DemoView: View {
        ...
        var body: some View {
            ...
            VStack {
                NotificationStack(colors, currentIndex: $currentIndex) { namedColor in
